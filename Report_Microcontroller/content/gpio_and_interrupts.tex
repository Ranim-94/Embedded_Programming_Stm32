...
\item Interrupt depend on the peripheral we are programming: like an GPIO interrupt is different than an I2C interrupt.

\item Interrupt doesn't not rely on pure software, there is some hardware role also $\rightarrow$ hardware software interaction
...
\item When we press the buttong, the \verb|EXTI| engine detect the transition (whether it is falling edge or rising edge)
	...
	\item At this stage, the bit inside \verb|EXTI->PR| for a particular pin number is set (this is pure hardware, caused by the event of pressing the button)
...
\item If the processor is not executing any other interrupt, it fetches the \tbi{vector addresse}  for the corresponding \verb|IRQ| number

	\begin{itemize}
	\item This vector address contains the address of the interrupt routine in which will be executed
	\end{itemize}


\item Inside the ISR, we do the following:

	\begin{itemize}
	\item Clear the pending bit of \verb|EXTI->PR| register
	
	\item Execute user code, like toggling a LED for example
